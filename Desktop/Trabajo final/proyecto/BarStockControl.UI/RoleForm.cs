    ...
    public partial class RoleForm : Form
    {
        ...
        private void btnCreate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var errors = _roleService.CreateRole(role);
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var role = GetRoleFromForm();
                role.Id = _selectedRole.Id;

                var errors = _roleService.UpdateRole(role);
                if (errors.Any())
                {
                    MessageBox.Show(string.Join("\n", errors), "Errores", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
        ...
        private void dgvRoles_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex >= 0)
                {
                    ...
                    {
                        if (clbPermissions.Items[i] is Permission p)
                        {
        ...
        private Role GetRoleFromForm()
        {
            ...
            {
                if (checkedItem is Permission p)
